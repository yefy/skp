...

{
    ...
    //int len;
    int len;
    unsigned char buffer[1024 * 64], digest[16];
    int i;
    char output1[32];
    ...
    { printf ("%s can't be openedn", filename);
        ...
        MD5Init (&context);
        while ((len = fread (buffer, 1, 1024 * 64, file)))
...
char *SkpMD5::string(char *string, unsigned int len)
{
    return MDString(string, len);
}

char *SkpMD5::file(char *string)
{
    return MDFile(string);
}

char *SkpMD5::mac(char* text, char* key)
{
    return hmac_md5(text, key);
}
