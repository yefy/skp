...
public :
    SkpRBTree();
    bool isEmpty();
    util_rbtree_node_t *insert(int64 key, void *data);
    void remove(util_rbtree_node_t *node);
    util_rbtree_node_t *min();
    util_rbtree_node_t *max();
    util_rbtree_node_t *search(int64 key);
    util_rbtree_node_t *lookup(int64 key);
    util_rbtree_t *rbtree();

private:
    util_rbtree_t *m_rbtree;
};
#endif // SKPRBTREE_H
