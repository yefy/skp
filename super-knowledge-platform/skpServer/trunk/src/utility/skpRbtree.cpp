...
{
    if(node == sentinel) return skp_null;
...
void util_rbtree_insert(util_rbtree_t *rbtree, util_rbtree_node_t *node)
{
    ...
    if((rbtree==skp_null) || (node==skp_null) || (node==_NULL(rbtree)))
	{
		return;
	}
    /* the tree is empty */
...
util_rbtree_node_t* util_rbtree_search(util_rbtree_t *rbtree, int64 key)
{
    if(rbtree != skp_null)
	{
...
util_rbtree_node_t* util_rbtree_lookup(util_rbtree_t *rbtree, int64 key)
{
    if((rbtree != skp_null) && !util_rbtree_isempty(rbtree))
	{
        util_rbtree_node_t *node = skp_null;
        ...
        return ((node != skp_null) ? node : util_rbtree_min(rbtree));
	}
    return skp_null;
}
...
SkpRBTree::SkpRBTree()
{
    m_rbtree = new util_rbtree_s();
    util_rbtree_init(m_rbtree);
}

bool SkpRBTree::isEmpty()
{
    return util_rbtree_isempty(m_rbtree);
}

util_rbtree_node_t *SkpRBTree::insert(int64 key, void *data)
{
    SKP_ASSERT(data);

    util_rbtree_node_t *node = new util_rbtree_node_t();
    SKP_ASSERT(node);

    node->key = key;
    node->data = data;
    util_rbtree_insert(m_rbtree, node);

    return node;
}

void SkpRBTree::remove(util_rbtree_node_t *node)
{
    util_rbtree_delete(m_rbtree, node);
}

util_rbtree_node_t *SkpRBTree::min()
{
    return util_rbtree_min(m_rbtree);
}

util_rbtree_node_t *SkpRBTree::max()
{
    return util_rbtree_max(m_rbtree);
}

util_rbtree_node_t *SkpRBTree::search(int64 key)
{
    return util_rbtree_search(m_rbtree, key);
}

util_rbtree_node_t *SkpRBTree::lookup(int64 key)
{
    return util_rbtree_lookup(m_rbtree, key);
}

util_rbtree_t *SkpRBTree::rbtree()
{
    return m_rbtree;
}
