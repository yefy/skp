...
///Qt::CustomizeWindowHint | Qt::WindowMinimizeButtonHint
  mWidget->setWindowFlags(Qt::FramelessWindowHint|Qt::WindowSystemMenuHint);
#endif

  //mWidget->setAutoFillBackground(false);
  //mWidget->setAttribute(Qt::WA_TranslucentBackground,true);

  //mWidget->setWindowFlags(mWidget->windowFlags() |Qt::CustomizeWindowHint| Qt::FramelessWindowHint | Qt::WindowSystemMenuHint);
  //mWidget->setWindowFlags((mWidget->windowFlags() |  Qt::FramelessWindowHint) & ~Qt::CustomizeWindowHint);
  //Bug fix, mouse move events does not propagate from child widgets.
  //so need the hover events.

  //mWidget->setWindowOpacity(0.5);
  mWidget->setAttribute(Qt::WA_TranslucentBackground); //不被绘制上的部分设置透明
